...
class MessageFactory:
    ...
    def get_message(message_type, object):
        if type(message_type) is not str:
            raise NotImplementedError('Type of argument wrong it has to be a String')
        if message_type == 'KEY_INIT':
            object.type = 0
            message = KeyInitMessage(object)
        elif message_type == 'KEY_REPLY':
            object.type = 1
            message = KeyReplyMessage(object)
...
class RelayMessage(MessageBase):
    def __init__(self, values):
        ...
        self.message = self.add_padding(values.message, len_message + 8 - len_message % 8)
    ...
    def decode(self, value):
        ...
        self.version, self.type, length = self.get_type_version_length(value)
        self.key_id = self.remove_padding(str(value[4:8]))
        self.message = self.remove_padding(str(value[8:]))
...
class ErrorMessage(MessageBase):
    ...
    def encode(self):
        ...
        answer += PADDING_CHR
        answer += self.add_padding(str(self.compute_length()), 2)
        answer += str(self.error_code)# the size of the error code it's just one byte
        answer += PADDING_CHR
...
test_key_init_message = message.decode(test_key_init)
